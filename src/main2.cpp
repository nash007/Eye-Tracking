...
int main( int argc, const char** argv ) {
	...
	if( capture ) {
		while( true ) {
			...
			cv::Mat smallFrame;
			cv::resize(frame,smallFrame,cv::Size(round(0.8*frame.cols), round(0.8*frame.rows)),0.8,0.8,cv::INTER_LANCZOS4);
...
void findEyes(cv::Mat frame_gray, cv::Rect face) {
	...
	std::cout<<"The Left pupil is at "<<leftPupil<<std::endl;
	std::cout<<"The Right pupil is at "<<rightPupil<<std::endl;
	// get corner regions
